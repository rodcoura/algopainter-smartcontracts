...
contract.only('AlgoPainterToken', accounts => {
  it('should add account[2] as a white list manager', async () => {
    ...
    await timelock.schedulePayment(accounts[1], await timelock.addSeconds(ref, 21), web3.utils.toWei('3', 'ether'));

    console.log('Waiting 10s to first payment');
    sleep.sleep(10);

    await timelock.requestPayment({ from: accounts[1] });
    let remainingAmount = await await timelock.getRemainingAmount(accounts[1]);
